...
def make(root):
    ...
    for e in ch.state()['events']:
        if e.get('commit'):lines += [r'\paragraph{'+tex(e['stage'])+'}',r'\href{https://github.com/alexthegreat714/alex-blythe-site/commit/'+e['commit']+r'}{\texttt{'+e['commit']+r'}}']
    lines += [r'\par',tex('Hashes establish identity, not scientific validity or an immutable timestamp against repository administrators. The original frozen prediction and its failures remain preserved. Any later tuning must be published as a separately versioned follow-up.'),r'\end{document}']
